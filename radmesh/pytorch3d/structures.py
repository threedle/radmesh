...
class Meshes(Sequence["Meshes"]):
    ...
    def __init__(
        ...
        *,
        ...
        self.valid: torch.Tensor = None  # type: ignore
        ...
        self._verts_list: List[torch.Tensor] = None  # type: ignore
        self._faces_list: List[torch.Tensor] = None  # type: ignore

        # Packed representation for verts.
        self._verts_packed: torch.Tensor = None  # (sum(V_n), 3) # type: ignore
        self._verts_packed_to_mesh_idx: torch.Tensor = None  # sum(V_n) # type: ignore

        # Index to convert verts from flattened padded to packed
        self._verts_padded_to_packed_idx: torch.Tensor = None  # N * max_V  # type: ignore

        # Index of each mesh's first vert in the packed verts.
        # Assumes packing is sequential.
        self._mesh_to_verts_packed_first_idx: torch.Tensor = None  # N  # type: ignore

        # Packed representation for faces.
        self._faces_packed: torch.Tensor = None  # (sum(F_n), 3)  # type: ignore
        self._faces_packed_to_mesh_idx: torch.Tensor = None  # sum(F_n)  # type: ignore

        # Index of each mesh's first face in packed faces.
        # Assumes packing is sequential.
        self._mesh_to_faces_packed_first_idx: torch.Tensor = None  # N  # type: ignore
        ...
        self._edges_packed: torch.Tensor = None  # (sum(E_n), 2)  # type: ignore

        # Map from packed edges to corresponding mesh index.
        self._edges_packed_to_mesh_idx: torch.Tensor = None  # sum(E_n)  # type: ignore
        self._num_edges_per_mesh: torch.Tensor = None  # N  # type: ignore
        self._mesh_to_edges_packed_first_idx: torch.Tensor = None  # N  # type: ignore
        ...
        self._faces_packed_to_edges_packed: torch.Tensor = (
            None  # (sum(F_n), 3)  # type: ignore
        )

        # Padded representation of verts.
        self._verts_padded: torch.Tensor = None  # (N, max(V_n), 3)  # type: ignore
        self._num_verts_per_mesh: torch.Tensor = None  # N  # type: ignore

        # Padded representation of faces.
        self._faces_padded: torch.Tensor = None  # (N, max(F_n), 3)  # type: ignore
        self._num_faces_per_mesh: torch.Tensor = None  # N  # type: ignore

        # Face areas
        self._faces_areas_packed: torch.Tensor = None  # type: ignore

        # Normals
        self._verts_normals_packed: torch.Tensor = None  # type: ignore
        self._faces_normals_packed: torch.Tensor = None  # type: ignore

        # Packed representation of Laplacian Matrix
        self._laplacian_packed: torch.Tensor = None  # type: ignore
        ...
        elif isinstance(verts, torch.Tensor) and isinstance(faces, torch.Tensor):
        ...
        if textures is not None:
            raise NotImplementedError("no textures in this shim")
    ...
    def _set_verts_normals(self, verts_normals) -> None:
        if isinstance(verts_normals, list):
            ...
            self._verts_normals_packed = StructUtils.padded_to_packed(
    ...
    def __getitem__(self, index: Union[int, List[int], slice, torch.Tensor]) -> "Meshes":
        """
        Args:
    ...
    def isempty(self) -> bool:
        ...
        """
        return self._N == 0 or bool(self.valid.eq(False).all().item())
    ...
    def verts_list(self) -> List[torch.Tensor]:
        ...
        if self._verts_list is None:
            ...
            self._verts_list = StructUtils.padded_to_list(
    ...
    def faces_list(self) -> List[torch.Tensor]:
        ...
        if self._faces_list is None:
            ...
            self._faces_list = StructUtils.padded_to_list(
    ...
    def verts_packed_to_mesh_idx(self) -> torch.Tensor:
        """
    ...
    def mesh_to_verts_packed_first_idx(self) -> torch.Tensor:
        """
    ...
    def num_verts_per_mesh(self) -> torch.Tensor:
        """
    ...
    def faces_packed_to_mesh_idx(self) -> torch.Tensor:
        """
    ...
    def mesh_to_faces_packed_first_idx(self) -> torch.Tensor:
        """
        Return a 1D tensor x with length equal to the number of meshes such that
    ...
    def faces_padded(self) -> torch.Tensor:
        """
    ...
    def num_faces_per_mesh(self) -> torch.Tensor:
        """
        Return a 1D tensor x with length equal to the number of meshes giving
    ...
    def edges_packed_to_mesh_idx(self) -> torch.Tensor:
        """
    ...
    def mesh_to_edges_packed_first_idx(self) -> torch.Tensor:
        """
        Return a 1D tensor x with length equal to the number of meshes such that
    ...
    def faces_packed_to_edges_packed(self) -> torch.Tensor:
        """
    ...
    def num_edges_per_mesh(self) -> torch.Tensor:
        """
        Return a 1D tensor x with length equal to the number of meshes giving
    ...
    def verts_padded_to_packed_idx(self) -> torch.Tensor:
        ...
        self._verts_padded_to_packed_idx = torch.cat(
            [
                torch.arange(v.item(), dtype=torch.int64, device=self.device) + i * self._V
    ...
    def verts_normals_list(self) -> List[torch.Tensor]:
        ...
        return StructUtils.packed_to_list(verts_normals_packed, split_size)

    def verts_normals_padded(self) -> torch.Tensor:
        """
        ...
        return StructUtils.list_to_padded(
    ...
    def faces_normals_list(self) -> List[torch.Tensor]:
        ...
        return StructUtils.packed_to_list(faces_normals_packed, split_size)

    def faces_normals_padded(self) -> torch.Tensor:
        """
    ...
    def _compute_face_areas_normals(self, refresh: bool = False):
        ...
        if not (
            refresh
            or any(
                v is None for v in [self._faces_areas_packed, self._faces_normals_packed]
            )
        ):
            return
        ...
        face_normals_raw = torch.cross(
            vertices_faces[:, 2] - vertices_faces[:, 1],
            vertices_faces[:, 0] - vertices_faces[:, 1],
            dim=1,
        )
        face_areas = face_normals_raw / 2
        face_normals = torch.nn.functional.normalize(face_normals_raw, eps=1e-6, dim=1)
    ...
    def _compute_padded(self, refresh: bool = False):
        ...
        if self.isempty():
            ...
            self._faces_padded = StructUtils.list_to_padded(
                faces_list, (self._F, 3), pad_value=-1.0, equisized=self.equisized
            )
            self._verts_padded = StructUtils.list_to_padded(
    ...
    def _compute_packed(self, refresh: bool = False):
        ...
        verts_list_to_packed = StructUtils.list_to_packed(verts_list)
        ...
        faces_list_to_packed = StructUtils.list_to_packed(faces_list)
    ...
    def offset_verts_(self, vert_offsets_packed):
        ...
        if update_normals and any(
            v is not None for v in [self._faces_areas_packed, self._faces_normals_packed]
        ):
